...
def prepare_dataset(path, tokenizer):
    ...
    responses_text = [f"{record['output']}{tokenizer.eos_token}" for record in raw_data]
    ...
    input_lens = [input_tokens.ne(tokenizer.pad_token_id).sum().item for input_tokens in inputs_tokens]
